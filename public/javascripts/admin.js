...
$(document).on('click', '#updateRestaurant', function () {
	...
	let workingHoursStart = $('span[data-target=workingHoursStart]')
		.text()
		.trim();
...
socket.on('customer-notification', (data) => {
	$.notify(
		`Received order from ${data.customer}
        Requested delivery time: ${data.requestedDeliveryTime}
		Payment type: ${data.paymentType}
        Total price: ${data.price} KM`,
		{
			className: 'info',
...
socket.on('courier-notification', (data) => {
	$.notify(
		...
		delivered order for ${data.customer}
